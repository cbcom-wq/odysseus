...
export function buildJsonSchema(allowedKinds: readonly CardKind[]): unknown {
  ...
  const option = {
    ...
    properties: {
      ...
      returnDepartTime: { type: ['string', 'null'], pattern: '^\\d{2}:\\d{2}$' },
      returnArriveTime: { type: ['string', 'null'], pattern: '^\\d{2}:\\d{2}$' },
      returnOvernight: nullableBoolean,
      returnDurationMinutes: nullableNumber,
